...
( clk, rst_n, s_axi_awaddr, s_axi_awvalid, s_axi_awready, s_axi_wdata, s_axi_wstrb, s_axi_wvalid, s_axi_wready, s_axi_bresp, s_axi_bvalid, s_axi_bready, s_axi_araddr, s_axi_arvalid, s_axi_arready, s_axi_rdata, s_axi_rresp, s_axi_rvalid, s_axi_rready, s_axis_tdata, s_axis_tvalid, s_axis_tready, s_axis_tlast, m_axis_tdata, m_axis_tvalid, m_axis_tready, m_axis_tlast );
  ...
  logic phasea_done_all, softmax_done_hw;
  ...
  logic writeback_active, writeback_launch, writeback_completed, writeback_pending;
  logic sm_state_load;
  ...
  logic [31:0] p_store [Q_MICROTILES][KV_SUBBLOCKS][TILE_ROWS][TILE_COLS];
  logic [31:0] corr_store [Q_MICROTILES][KV_SUBBLOCKS][TILE_ROWS];
  logic [31:0] phaseb_delta [TILE_ROWS][HEAD_DIM];
  logic [31:0] obuf_corr_sel [TILE_ROWS];
  logic [31:0] obuf_l_sel [TILE_ROWS];
  logic        obuf_bank_sel_runtime;
  assign phasea_window = busy && !mac_phase && !mac_start && !softmax_start && !q_load_start && !kv_load_start && !o_write_start;
  assign phaseb_window = busy && mac_phase && !mac_start && !softmax_start && !q_load_start && !kv_load_start && !o_write_start;
  assign phasea_depth_active = (phasea_state == PA_RUN);
  assign phasea_depth_last = (depth_cnt == HEAD_DIM - 1);
  logic axis_valid; logic [15:0] axis_data; logic axis_last; logic [1:0] axis_dest; logic axis_done;
  logic src_valid, src_last; logic [15:0] src_data; logic src_done;
  logic buf_sel, q_bank_fill_ready, o_bank_sel; logic [15:0] q_buf_rd;
  logic [15:0] q_block_rd [TILE_ROWS];
  logic [4:0] q_rd_row, q_rd_row_start; logic [6:0] q_rd_dim;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    ...
    if (!rst_n) begin
      ...
      for (ctxm = 0; ctxm < Q_MICROTILES; ctxm++) begin
        for (ctxi = 0; ctxi < TILE_ROWS; ctxi++) begin
          ...
          for (ctxj = 0; ctxj < KV_SUBBLOCKS; ctxj++) begin
            ...
            for (ctxk = 0; ctxk < TILE_COLS; ctxk++) begin
              p_store[ctxm][ctxj][ctxi][ctxk] <= 32'd0;
            end
          end
        end
      end
      ...
      case (phasea_state)
        ...
        PA_WAIT_P: begin
          if (p_valid) begin
            if (phasea_kv_blk_idx == kv_subblocks_active-1) begin
              phasea_kv_blk_idx <= 2'd0;
              if (phasea_micro_idx == q_microtiles_active-1) begin
                phasea_state <= PA_IDLE;
              end else begin
      ...
      if (p_valid) begin
        ...
        for (ctxi = 0; ctxi < TILE_ROWS; ctxi++) begin
          ...
          for (ctxj = 0; ctxj < TILE_COLS; ctxj++) begin
            p_store[phasea_pending_micro][phasea_pending_kv_blk][ctxi][ctxj] <= p_block[ctxi][ctxj];
          end
        end
      end
    end
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    ...
    if (!rst_n) begin
      ...
      writeback_completed <= 1'b0;
      writeback_pending <= 1'b0;
      for (ctxi = 0; ctxi < TILE_ROWS; ctxi++) begin
        for (ctxj = 0; ctxj < HEAD_DIM; ctxj++) begin
          phaseb_delta[ctxi][ctxj] <= 32'd0;
        end
      end
    end else begin
      writeback_launch <= 1'b0;
      if (!o_write_start) begin
        writeback_completed <= 1'b0;
        writeback_pending <= 1'b0;
      end

      if (o_write_start && !writeback_active && !writeback_completed) begin
        writeback_active <= 1'b1;
        phaseb_norm_micro_idx <= 1'd0;
        writeback_launch <= 1'b1;
      end else if (writeback_pending) begin
        writeback_pending <= 1'b0;
        writeback_launch <= 1'b1;
      end else if (writeback_active &&
                   src_valid &&
                   (obuf_o_row == 5'(TILE_ROWS - 1)) &&
                   (obuf_o_dim == 7'(HEAD_DIM - 1))) begin
        if (src_done) begin
          writeback_active <= 1'b0;
          writeback_completed <= 1'b1;
        end else if (obuf_valid_raw &&
                     (obuf_o_row == 5'(TILE_ROWS - 1)) &&
                     (obuf_o_dim == 7'(HEAD_DIM - 1)) &&
                     (phaseb_norm_micro_idx != q_microtiles_active-1)) begin
      ...
      if (!phaseb_window) begin
        ...
        case (phaseb_state)
          PB_IDLE: begin
            ...
            if (kv_tile_first)
              phaseb_state <= PB_CLEAR;
            else
              phaseb_state <= PB_PRIME;
          end
          PB_CLEAR: begin
            for (ctxi = 0; ctxi < TILE_ROWS; ctxi++) begin
              for (ctxj = 0; ctxj < HEAD_DIM; ctxj++) begin
          ...
          PB_RUN: begin
            if (phaseb_k_idx == TILE_COLS-1)
              phaseb_state <= PB_FLUSH;
            else
          ...
          PB_CAPTURE: begin
            for (ctxi = 0; ctxi < TILE_ROWS; ctxi++) begin
              for (ctxj = 0; ctxj < TILE_COLS; ctxj++) begin
                phaseb_delta[ctxi][phaseb_dim_blk_idx * TILE_COLS + ctxj] <= mac_block_out[ctxi][ctxj];
              end
            end
            if (phaseb_dim_blk_idx == DIM_SUBBLOCKS-1) begin
              phaseb_dim_blk_idx <= 3'd0;
              phaseb_row_update_idx <= 5'd0;
          ...
          PB_UPDATE: begin
            if (phaseb_row_update_idx == TILE_ROWS-1) begin
              phaseb_row_update_idx <= 5'd0;
              phaseb_dim_blk_idx <= 3'd0;
              if (phaseb_kv_blk_idx == kv_subblocks_active-1) begin
                phaseb_kv_blk_idx <= 2'd0;
                if (phaseb_micro_idx == q_microtiles_active-1) begin
          ...
          default: phaseb_state <= PB_IDLE;
        endcase
      end
    end
  end

  assign q_microtiles_active = (active_q_rows > TILE_ROWS) ? 2'd2 : 2'd1;
  assign kv_subblocks_active = (active_kv_cols + TILE_COLS - 1) >> 4;
  assign phasea_done_all = (phasea_p_capture_cnt == q_microtiles_active * kv_subblocks_active);
  assign phaseb_done_all = (phaseb_state == PB_DONE);

  always_comb begin
    int phasea_row_base;
    int phaseb_row_base;
    int writeback_row_base;
    int phasea_rows_left;
    int phaseb_rows_left;
    int writeback_rows_left;
    int phasea_cols_left;
    phasea_row_base = phasea_micro_idx * TILE_ROWS;
    phaseb_row_base = phaseb_micro_idx * TILE_ROWS;
    writeback_row_base = phaseb_norm_micro_idx * TILE_ROWS;
    phasea_rows_left = active_q_rows - phasea_row_base;
    phaseb_rows_left = active_q_rows - phaseb_row_base;
    writeback_rows_left = active_q_rows - writeback_row_base;
    phasea_cols_left = active_kv_cols - phasea_kv_blk_idx * TILE_COLS;
    if (phasea_rows_left <= 0)
      phasea_softmax_active_rows = 5'd0;
    else if (phasea_rows_left >= TILE_ROWS)
      phasea_softmax_active_rows = TILE_ROWS;
    else
      phasea_softmax_active_rows = phasea_rows_left[4:0];
    if (phaseb_rows_left <= 0)
      phaseb_active_rows = 5'd0;
    else if (phaseb_rows_left >= TILE_ROWS)
      phaseb_active_rows = TILE_ROWS;
    else
      phaseb_active_rows = phaseb_rows_left[4:0];
    if (writeback_rows_left <= 0)
      writeback_active_rows = 5'd0;
    else if (writeback_rows_left >= TILE_ROWS)
      writeback_active_rows = TILE_ROWS;
    else
      writeback_active_rows = writeback_rows_left[4:0];
    if (phasea_cols_left <= 0)
      phasea_softmax_active_cols = 5'd0;
    else if (phasea_cols_left >= TILE_COLS)
      phasea_softmax_active_cols = TILE_COLS;
    else
      phasea_softmax_active_cols = phasea_cols_left[4:0];

    for (int ri = 0; ri < TILE_ROWS; ri++) begin
      if (!mac_phase)
        mac_row[ri] = (ri < phasea_softmax_active_rows) ? q_block_rd[ri] : 16'd0;
      else if (ri < phaseb_active_rows)
        mac_row[ri] = p_store[phaseb_micro_idx][phaseb_kv_blk_idx][ri][phaseb_k_idx][31:16];
      else
        mac_row[ri] = 16'd0;
    end
    ...
    for (int ri = 0; ri < TILE_ROWS; ri++) begin
      if (writeback_active || writeback_completed)
        obuf_l_sel[ri] = sm_l_ctx[phaseb_norm_micro_idx][ri];
      else
        obuf_l_sel[ri] = sm_l_ctx[phaseb_micro_idx][ri];

      if (phaseb_state == PB_UPDATE)
        obuf_corr_sel[ri] = corr_store[phaseb_micro_idx][phaseb_kv_blk_idx][ri];
      else
        obuf_corr_sel[ri] = 32'h3F80_0000;
    end

    for (int di = 0; di < HEAD_DIM; di++) begin
      obuf_data[di] = phaseb_delta[phaseb_row_update_idx][di];
    end
  end
  ...
  endgenerate
  assign mac_split=2'd2;
  assign mac_accum_en = phasea_depth_active || (phaseb_state == PB_RUN) || (phaseb_state == PB_FLUSH);
  assign obuf_update = (phaseb_state == PB_UPDATE) && (phaseb_row_update_idx < phaseb_active_rows);
  assign obuf_row = phaseb_row_update_idx;
  ...
  assign q_load_done = q_bank_ready[q_compute_bank_sel];
  ...
  assign obuf_clear_bank = (phaseb_state == PB_CLEAR);
  assign obuf_clear_bank_sel = phaseb_micro_idx;
  assign obuf_bank_sel_runtime = (writeback_active || writeback_completed)
  ...
  assign v_rd_vec_en = (phaseb_state == PB_PRIME) || (phaseb_state == PB_RUN && phaseb_k_idx != TILE_COLS-1);
  assign v_rd_vec_token_idx = kv_tile_start + {{10{1'b0}}, phaseb_kv_blk_idx, 4'd0} +
                              ((phaseb_state == PB_PRIME) ? 16'd0 : {12'd0, phaseb_k_idx + 4'd1});
  assign v_rd_vec_dim_start = {phaseb_dim_blk_idx, 4'd0};
  assign q_rd_row={phasea_micro_idx, 4'd0}; assign q_rd_row_start={phasea_micro_idx, 4'd0}; assign q_rd_dim=depth_cnt;
  assign mac_clear_accum = !mac_phase ? (depth_cnt == 7'd0) : ((phaseb_state == PB_RUN) && (phaseb_k_idx == 4'd0));
  ...
  assign src_last = (writeback_active_rows != 5'd0) &&
                    (phaseb_norm_micro_idx == q_microtiles_active-1) &&
                    (obuf_o_row == writeback_active_rows - 5'd1) && (obuf_o_dim == 7'(HEAD_DIM - 1));
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      v_loaded  <= 1'b0;
      q_bank_ready <= 2'b00;
      q_load_bank_sel <= 1'b0;
    end else begin
      if (k_wr_en) k_wr_addr <= k_wr_addr + 16'd1;
      if (v_wr_en) v_wr_addr <= v_wr_addr + 16'd1;
      if (qbuf_wr_en) q_wr_addr <= q_wr_addr + 16'd1;

      if (q_load_start)
        q_load_bank_sel <= mac_phase ? ~buf_sel : buf_sel;
      ...
      if (axis_done) begin
        case (axis_dest)
          ...
          STREAM_TO_Q_BUF: begin
            q_bank_ready[q_load_bank_sel] <= 1'b1;
            q_wr_addr <= 16'd0;
          end
          default: ;
        endcase
  ...
  attn_axi_lite_slave u_csr(.clk,.rst_n,.s_axi_awaddr,.s_axi_awvalid,.s_axi_awready,.s_axi_wdata,.s_axi_wstrb,.s_axi_wvalid,.s_axi_wready,.s_axi_bresp,.s_axi_bvalid,.s_axi_bready,.s_axi_araddr,.s_axi_arvalid,.s_axi_arready,.s_axi_rdata,.s_axi_rresp,.s_axi_rvalid,.s_axi_rready,.start,.seq_len,.gqa_group,.q_head,.stream_dest(stream_dest_cfg),.stream_len(stream_len_cfg),.result_len(result_len_cfg),.done,.cycle_cnt,.mac_cycles);
  attn_axi_stream_sink u_sink(.clk,.rst_n,.s_axis_tdata,.s_axis_tvalid,.s_axis_tready,.s_axis_tlast,.data_valid(axis_valid),.data_out(axis_data),.data_last(axis_last),.cfg_dest(stream_dest_cfg),.cfg_len(stream_len_cfg),.cfg_burst(4'd0),.dest_sel(axis_dest),.bytes_received(),.overflow(),.underflow(),.done(axis_done));
  attn_axi_stream_source u_src(.clk,.rst_n,.data_valid(src_valid),.data_in(src_data),.data_last(src_last),.cfg_len(result_len_cfg),.m_axis_tdata,.m_axis_tvalid,.m_axis_tready,.m_axis_tlast,.bytes_sent(),.done(src_done));
  attn_core u_fsm(.clk,.rst_n,.start,.start_ready(),.seq_len,.cfg_q_pos_base(16'd0),.cfg_kv_pos_base(16'd0),.cfg_causal(1'b1),.done,.busy,.kv_load_start,.kv_load_done,.q_load_start,.q_load_done,.o_write_start,.o_write_done,.buf_sel(buf_sel),.o_bank_sel(o_bank_sel),.group_advance(group_advance),.mac_phase,.mac_start,.mac_done,.softmax_start,.softmax_done,.kv_tile_first,.kv_tile_last,.q_tile_start(q_tile_start),.kv_tile_start(kv_tile_start),.active_q_rows(active_q_rows),.active_kv_cols(active_kv_cols),.error(),.cycle_cnt,.mac_cycles,.stall_cycles(),.perf_valid());
  kv_cache_ram u_kcache(.clk,.rst_n,.wr_en(k_wr_en),.wr_addr(k_wr_addr),.wr_data(axis_data),.rd_en(k_rd_en),.rd_token_start(k_rd_start),.rd_dim(k_rd_dim),.rd_data(k_rd),.rd_vec_en(1'b0),.rd_vec_token_idx(16'd0),.rd_vec_dim_start(7'd0),.rd_vec_data());
  ...
  tile_buffer u_qbuf(.clk,.rst_n,.wr_en(qbuf_wr_en),.wr_data(axis_data),.rd_en(phasea_depth_active),.rd_row(q_rd_row),.rd_row_start(q_rd_row_start),.rd_dim(q_rd_dim),.rd_data(q_buf_rd),.rd_block_data(q_block_rd),.wr_bank_sel(q_load_bank_sel),.rd_bank_sel(q_compute_bank_sel),.bank_ready(q_bank_fill_ready));
  ...
  softmax_engine u_softmax(.clk,.rst_n,.s_valid,.s_data(s_block),.kv_tile_first,.kv_tile_last,.causal_mask_en(1'b1),.q_tile_start(softmax_q_tile_start),.kv_tile_start(softmax_kv_tile_start),.active_rows(phasea_softmax_active_rows),.active_cols(phasea_softmax_active_cols),.state_load(sm_state_load),.state_m_in(sm_state_m_in),.state_l_in(sm_state_l_in),.m_state,.l_state,.p_valid,.p_data(p_block),.correction,.done(softmax_done_hw));
  ...
  output_buffer u_obuf(.clk,.rst_n,.clear_bank(obuf_clear_bank),.clear_bank_sel(obuf_clear_bank_sel),.acc_update(obuf_update),.acc_row(obuf_row),.acc_data(obuf_data),.correction(obuf_corr_sel),.bank_sel(obuf_bank_sel_runtime),.normalize(obuf_norm),.l_state(obuf_l_sel),.o_valid(obuf_valid_raw),.o_row(obuf_o_row),.o_dim(obuf_o_dim),.o_data(obuf_data_raw));
