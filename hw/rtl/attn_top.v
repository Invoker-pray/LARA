...
( clk, rst_n, s_axi_awaddr, s_axi_awvalid, s_axi_awready, s_axi_wdata, s_axi_wstrb, s_axi_wvalid, s_axi_wready, s_axi_bresp, s_axi_bvalid, s_axi_bready, s_axi_araddr, s_axi_arvalid, s_axi_arready, s_axi_rdata, s_axi_rresp, s_axi_rvalid, s_axi_rready, s_axis_tdata, s_axis_tvalid, s_axis_tready, s_axis_tlast, m_axis_tdata, m_axis_tvalid, m_axis_tready, m_axis_tlast );
  ...
  attn_core u_fsm(.clk,.rst_n,.start,.start_ready(),.seq_len,.cfg_q_pos_base(16'd0),.cfg_kv_pos_base(16'd0),.cfg_causal(1'b1),.done,.busy,.kv_load_start,.kv_load_done,.q_load_start,.q_load_done,.o_write_start,.o_write_done,.mac_phase,.mac_start,.mac_done,.softmax_start,.softmax_done,.kv_tile_first,.kv_tile_last,.q_tile_start(q_tile_start),.kv_tile_start(kv_tile_start),.active_q_rows(),.active_kv_cols(),.error(),.cycle_cnt,.mac_cycles);
  ...
  softmax_engine u_softmax(.clk,.rst_n,.s_valid,.s_data(s_block),.kv_tile_first,.kv_tile_last,.causal_mask_en(1'b1),.q_tile_start(q_tile_start),.kv_tile_start(kv_tile_start),.m_state,.l_state,.p_valid,.p_data(p_block),.correction,.done(softmax_done));
