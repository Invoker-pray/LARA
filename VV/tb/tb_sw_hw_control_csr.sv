...
module tb_sw_hw_control_csr;
  ...
  int clear_pulse_count = 0;
  always @(posedge clk) if (prefetch_error_clear) clear_pulse_count++;
  ...
  initial begin
    ...
    prefetch_supported=1; prefetch_enabled=0; prefetch_q_ready=1; prefetch_kv_ready=0;
    prefetch_underflow=0; prefetch_overflow=0; prefetch_outstanding=8'd2;
    ...
    // Prefetch capability/status. In the full design attn_top loops the CTRL
    // enable output back into the STATUS enabled input, so the TB drives that
    // input explicitly for each readback check.
    axi_read(CSR_PREFETCH_STATUS, rd);
    if (!rd[0] || rd[1] || !rd[2] || rd[3] || rd[4] || rd[5] || rd[15:8] != 8'd2)
      fail("prefetch status idle decode");
    axi_write(CSR_PREFETCH_CTRL, 32'd1);
    if (!prefetch_enable) fail("prefetch enable output");
    prefetch_enabled = 1; prefetch_kv_ready = 1; prefetch_overflow = 1;
    axi_read(CSR_PREFETCH_STATUS, rd);
    if (!rd[1] || !rd[3] || !rd[5]) fail("prefetch status live decode");
    prefetch_overflow = 0; prefetch_enabled = 0; prefetch_kv_ready = 0;
    // W1P error clear must pulse (observed by the monitor process below).
